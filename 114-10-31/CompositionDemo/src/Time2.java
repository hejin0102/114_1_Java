public class Time2 {
    ...
    // Time2 無參數建構子：將每個成員變數初始化為 0
    public Time2() {
        this(0, 0, 0); // 呼叫三參數建構子
    }

    // Time2 建構子：提供 hour，minute 與 second 預設為 0
    public Time2(int hour) {
        this(hour, 0, 0); // 呼叫三參數建構子
    }

    // Time2 建構子：提供 hour 與 minute，second 預設為 0
    ...
    // 以 24 小時格式轉為字串 (HH:MM:SS)
    public String toUniversalString() {
        return String.format(
                "%02d:%02d:%02d", getHour(), getMinute(), getSecond());
    }

    // 以 12 小時格式轉為字串 (H:MM:SS 上午或下午)
